...
def startGame(event):
        ...
        # start the countdown timer.
            # Mahesh Final Edit
            speaker.say("the game starts now")
            speaker.runAndWait()
            countdown()
...
def nextColour(value):
    ...
    if timeleft > 0:
        ...
        if score>=30:
            ...
            speaker.say("You have scored"+str(score)+"points,You are Well below average")
            speaker.runAndWait()
        else:
...
def countdown():
    ...
    if timeleft > 0:
        ...
        #timeout_label.pack()
        speaker.say("Time over,  please press enter button to get score along with review")
        speaker.runAndWait()
...
# create a GUI window
root = Tk()
timeout_label=Label(root,text="Time over,  please press enter button to get score along with review")
...
def sel():
    ...
    #Mahesh Final Edit
    if(var.get()==1):
        selectionq.config(text="You have choosen Easy Mode")
    else:
        selectionq.config(text="You have choosen Difficulty Mode")
    selectionq.pack()
    ...
    if var.get()==2:
        ...
        speaker.say("You have chosen easy mode")
        speaker.runAndWait()
        easymode = True
        difficultmode = False
...
label = Label(root, font=('Helvetica', 60),bd=3)
...
yesButton = Button(root, text = 'Yes !', bd = '10',width='20',fg='Blue',bg='light blue',font=buttonFont,command = lambda :nextColour(1))
noButton = Button(root, text = 'No !', bd = '10',width='20',fg='purple',bg='pink',font=buttonFont,command = lambda :nextColour(0))
